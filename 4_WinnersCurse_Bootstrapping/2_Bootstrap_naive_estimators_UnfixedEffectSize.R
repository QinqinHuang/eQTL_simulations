...
# Data under:
mydir <- "~/Simulation_eQTL_replicates/scenario_files/Bootstrap_reestimation_effect_size/BootstrapQTL_v0.7.3_eigenMT-BH_nomthres_200_UnfixedEffectSize/"
setwd(mydir)
...
writelog <- function(newinfo, IdxRep, SS, MAF) {
...
# Bootstrap results
bootsest <- foreach(IdxRep = rep(whichreps, each = 36), SS = rep(rep(c(5000,2000,1000,500,200,100), each = 6), howmanyreps), MAF = rep(c(50,25,10,5,1,0.5), 6*howmanyreps), .combine = rbind ) %dopar% {
  ...
  returndd <- topSNP200true[,c(paste0("TP_",localcormet,globalcormet),"sim_beta","nominal_beta")]
  ...
  returndd <- returndd[,-1]
  colnames(returndd)[2] <- "estimates"
  # scenario parameters
  returndd$Rep <- IdxRep; returndd$SS <- SS; returndd$MAF <- MAF
  ...
  bootsfilename <- paste0("Rep",IdxRep,"SS",SS,"MAF",MAF,"_BootstrapQTL200_UnfixedEffectSize_output.txt")
  if(!file.exists(bootsfilename)) {
    writelog(newinfo = "Error - no BootstrapQTL output.", IdxRep, SS, MAF)
    return(NULL)
  }
  ...
  if(sum(topSNP[,paste0("sig_",localcormet,globalcormet)]) != nrow(bootsout_top)) {  writelog(newinfo = "Error - BootstrapQTL not the same number of significant eGenes.", IdxRep, SS, MAF) }
  # (ii). top SNPs and nominal betas
  if(!identical( formatC(topSNP$nominal_beta[match(bootsout_top$eGene, topSNP$gene)], digits = 5), formatC(bootsout_top$nominal_beta, digits = 5) )) { writelog(newinfo = "Error - nominal betas are not identical.", IdxRep, SS, MAF) }
  ...
  bootsout_TP <- merge(bootsout_TP, topSNP200true[,c("gene","sim_beta")], by.x = "eGene", by.y = "gene")
  if(n_TP != nrow(bootsout_TP)) { writelog(newinfo = "Error - BootstrapQTL not the same number of TPs.", IdxRep, SS, MAF) }
  
  # 2. Bootstrap estimator for True Positive eGenes
  bootest <- bootsout_TP[,c("sim_beta","shrinkage","outof","weighted")]
  bootest <- melt(bootest, id.vars = "sim_beta")
  colnames(bootest)[-1] <- c("Estimator","estimates")
  bootest$Rep <- IdxRep; bootest$SS <- SS; bootest$MAF <- MAF
