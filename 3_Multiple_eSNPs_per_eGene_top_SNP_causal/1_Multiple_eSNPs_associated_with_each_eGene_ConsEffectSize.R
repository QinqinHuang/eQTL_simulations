...
num_eAsso_allscenarios <- foreach( IdxRep = rep(whichreps, each = 36*4), SS = rep(rep(c(5000,2000,1000,500,200,100), each = 6*4), howmanyreps), MAF = rep(rep(c(50,25,10,5,1,0.5), each = 4), 6*howmanyreps), effectsize = rep(c(0.25, 0.5, 1, 1.5), 36*howmanyreps), .combine = rbind ) %dopar% {
  ...
  ddreturn <- data.frame(Rep = IdxRep, SS = SS, MAF = MAF, effectsize = effectsize)
  # Number of TP eGenes
...
sum_data <- calculate_sum_ignoreNAs(num_eAsso_allscenarios, c("SS","MAF","effectsize"))
...
ave_nSNP <- sum_data[,c("SS","MAF","effectsize","n_SNP_TPeGene","n_SNP_sigeGene")]

# Save results
setwd(mydir)
# Add the multiple testing correction method name
colnames(num_eAsso_allscenarios)[5:8] <- paste0(colnames(num_eAsso_allscenarios)[5:8],"_",localcormet,"_",globalcormet)
colnames(ave_nSNP)[4:5] <- paste0(c("n_SNP_TPeGene","n_SNP_sigeGene"),"_",localcormet,"_",globalcormet)
