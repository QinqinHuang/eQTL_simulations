...
num_top_causal <- foreach(IdxRep = rep(whichreps, each = 36), SS = rep(rep(c(5000,2000,1000,500,200,100), each = 6), howmanyreps), MAF = rep(c(50,25,10,5,1,0.5), 6*howmanyreps), .combine = rbind) %dopar% {
  # Read the info about top SNPs
  output_filename <- paste0("./output_files/Rep",IdxRep,"SS",SS,"MAF",MAF,"_",typeeffectsize,"_Top_SNP_all_genes_causalSNP_info_",localcormet,globalcormet,".txt")
  ...
  # Return table
  returndd <- data.frame(Rep = IdxRep, SS = SS, MAF = MAF)
...
sumrep_num_top_causal <- calculate_sum_ignoreNAs(num_top_causal, scenario_parameters = c("SS","MAF"))
...
LDr2_allscen <- foreach(IdxRep = rep(whichreps, each = 36), SS = rep(rep(c(5000,2000,1000,500,200,100), each = 6), howmanyreps), MAF = rep(c(50,25,10,5,1,0.5), 6*howmanyreps), .combine = rbind) %dopar% {
  # Read the info about top SNPs
  output_filename <- paste0("./output_files/Rep",IdxRep,"SS",SS,"MAF",MAF,"_",typeeffectsize,"_Top_SNP_all_genes_causalSNP_info_",localcormet,globalcormet,".txt")
  topSNP <- read.table(output_filename, header = T)
  # TP eGenes
